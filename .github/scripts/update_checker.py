            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        # 如果所有尝试都失败，返回当前版本作为后备方案
                        print("使用后备方案：返回当前已知的最新版本")
                        return "1.0.2"  # 当前已知的FreeCAD版本
    ...
    def update_yaml_file(self, yaml_file, new_version):
        ...
        try:
            ...
            for source in sources:
                ...
                # 根据架构调整URL模板
                if arch == "aarch64":
                    # arm64架构需要特殊处理，可能需要不同的URL格式
                    # 检查模板是否包含架构占位符
                    if "{arch}" in self.download_url_template:
                        new_url = self.download_url_template.format(
                            version=new_version, arch="arm64"
                        )
                    else:
                        # 如果模板不包含架构信息，为arm64添加架构后缀
                        new_url = self.download_url_template.format(
                            version=new_version, arch=arch
                        ).replace(".AppImage", "-arm64.AppImage")
                else:
                    # x86_64架构使用标准格式
                    new_url = self.download_url_template.format(
                        version=new_version, arch=arch
